...
struct SignerApp {
    ...
    verify_sig_path: Option<PathBuf>,
...
impl SignerApp {
    fn ui_keygen(&mut self, ui: &mut egui::Ui) {
        ...
        ui.add(egui::TextEdit::singleline(&mut self.keygen_pass).password(true).hint_text("Passphrase"));
        ui.add(egui::TextEdit::singleline(&mut self.keygen_confirm).password(true).hint_text("Confirm Passphrase"));
    ...
    fn ui_sign(&mut self, ui: &mut egui::Ui) {
        if ui.button("Select Encrypted Key").clicked() {
            if let Some(path) = FileDialog::new().add_filter("Key", &["key"]).pick_file() {
        ...
        ui.add(egui::TextEdit::singleline(&mut self.sign_pass).password(true).hint_text("Key Passphrase"));
        ...
        if ui.button("Sign File").clicked() {
            if let (Some(key_path), Some(file_path)) = (&self.sign_key_path, &self.sign_file_path) {
                ...
                match key_content_res {
                    Ok(content) => {
                         let parse_res: Result<keystore::EncryptedKeyFile, _> = serde_json::from_str(&content);
                         match parse_res {
                             Ok(enc_key) => {
                                 let decrypt_res = crypto::decrypt_private_key(&enc_key, &self.sign_pass);
                                 match decrypt_res {
                                     Ok(signing_key) => {
                                         match std::fs::read(file_path) {
                                             Ok(data) => {
                                                 ...
                                                 if let Ok(_) = std::fs::write(&sig_path, hex::encode(sig.to_bytes())) {
                                                      self.sign_status = format!("✅ Signed: {}", sig_path);
                                                 } else {
                                                      self.sign_status = "❌ Failed to write signature".to_string();
                                                 }
                                             }
                                             Err(e) => self.sign_status = format!("❌ File read error: {}", e),
                                         }
                                     }
                                     Err(_) => self.sign_status = "❌ Wrong password or corrupt key".to_string(),
                                 }
                             }
    ...
    fn ui_verify(&mut self, ui: &mut egui::Ui) {
        ...
        if ui.button("Verify Signature").clicked() {
             if let (Some(key_path), Some(file_path)) = (&self.verify_key_path, &self.verify_file_path) {
                 ...
                 if sig_path.exists() {
                     ...
                     if let (Ok(pub_hex), Ok(sig_hex), Ok(data)) = (pub_res, sig_res, data_res) {
                         if let (Ok(pub_bytes), Ok(sig_bytes)) = (hex::decode(pub_hex.trim()), hex::decode(sig_hex.trim())) {
                              use ed25519_dalek::{VerifyingKey, Signature, Verifier};
                              let vk_res = VerifyingKey::from_bytes(&pub_bytes.try_into().unwrap_or([0;32]));
                              let sig = Signature::from_bytes(&sig_bytes.try_into().unwrap_or([0;64])); 
                              
                              if let Ok(vk) = vk_res {
                                  match vk.verify(&data, &sig) {
                                      Ok(_) => self.verify_status = "✅ Valid Signature!".to_string(),
                                      Err(_) => self.verify_status = "❌ Invalid Signature".to_string(),
                                  }
                              } else {
