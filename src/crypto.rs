...
pub fn decrypt_private_key(encrypted: &EncryptedKeyFile, password: &str) -> Result<SigningKey> {
    ...
    let signing_key_bytes: [u8; 32] = plaintext.try_into().map_err(|_| anyhow::anyhow!("Invalid key length"))?;
...
pub fn verify_signature(key: &VerifyingKey, data: &[u8], sig: &Signature) -> Result<()> {
    key.verify(data, sig).map_err(|e| anyhow::anyhow!("Verification failed: {}", e))
}
